...
# Локальное время для Республики Беларусь (Минск, UTC+3)
BELARUS_OFFSET = timedelta(hours=3)

def _now():
    # Используем UTC+3 как локальное время Минска, без таймзоны в строке
    return (datetime.utcnow() + BELARUS_OFFSET).strftime("%Y-%m-%dT%H:%M:%S")
