...
def _sign(params: dict) -> str:
    ...
    secret = EP_SECRET_WORD.encode("utf-8")
    sig    = hmac.new(secret, raw.encode("utf-8"), hashlib.sha1)
...
async def create_card_invoice(
    ...
    if resp.status_code not in (200, 201):
        logger.error(f"Express-Pay cardinvoices error {resp.status_code}: {resp.text}")
...
async def get_card_invoice_status(invoice_no: int) -> dict:
    """
    Запрашивает статус счёта.
    Express-Pay возвращает два номера: InvoiceNo и CardInvoiceNo.
    Для запроса статуса используем CardInvoiceNo (он же передаётся в URL).
    """
...
async def get_qr_code_base64(invoice_no: int) -> Optional[str]:
    ...
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        return data.get("QrCodeBody")
    except Exception as exc:
        logger.warning(f"Express-Pay QR error: {exc}")
        return None
...
async def run_startup_test() -> dict:
    ...
    try:
        result = await create_card_invoice(
            ...
            amount=1.00,
            info="Автоматический тест при запуске FastAPI-ExpressPay",
            return_url=f"{BASE_URL}/payment/success",
            fail_url=f"{BASE_URL}/payment/fail",
        )
        # Express-Pay возвращает InvoiceNo и CardInvoiceNo — сохраняем оба
        invoice_no     = result.get("InvoiceNo")
        card_invoice_no = result.get("CardInvoiceNo")
        form_url       = result.get("FormUrl", "") or None  # "" → None
        ok             = invoice_no is not None

        return {
            "account_no":      account_no,
            "ok":              ok,
            "invoice_no":      invoice_no,
            "card_invoice_no": card_invoice_no,
            "form_url":        form_url,
            "raw":             result,
        }
    except Exception as exc:
